...
echo.
Echo Removing Windows Defender + Security Mitigations
timeout /t 1 >Nul
Pause
CLS
Title  Removing Windows Security UWP App...
...
CLS
Title Unregister Windows Defender Security Components...
...
bcdedit /set hypervisorlaunchtype off
FOR /R %%f IN (Remove_SecurityComp\*.reg) DO PowerRun.exe regedit.exe /s "%%f"
echo Your PC Need Reboot BTW ...
Echo ( AFTER ALL IF YOU WANT )
timeout 2 >Nul
